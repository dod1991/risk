...
def compute_quant_score(df):
    ...
    # TREND STRATEGY ONLY
    # ========================================
    trend_quant = 0.6 * momentum + 0.4 * trend

    # align with direction
    trend_quant = trend_quant * trend_dir

    # ========================================
    # CHOP = NO TRADE
    # ========================================
    df["quant_raw"] = np.where(
        regime == "trend",
        trend_quant,
        0
    )
...
def compute_confidence(df):
    ...
    df["quant_confidence"] = df["quant_score"].abs()
...
def apply_quant_agent(df):
    ...
    print("\n--- RUNNING QUANT AGENT (TREND ONLY) ---\n")
